 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_lticoursetemplate_helper_testcase extends advanced_testcase {
    ...
    public function test_update_user_profile_image() {
        ...
        \enrol_lticoursetemplate\helper::update_user_profile_image($this->user1->id, $this->getExternalTestFileUrl('/test.jpg'));
    ...
    public function test_enrol_user_max_enrolled() {
        ...
        $tool = \enrol_lticoursetemplate\helper::get_lti_tool($tool->id);

        // Enrol a user.
        $result = \enrol_lticoursetemplate\helper::enrol_user($tool, $this->user1->id);
        ...
        $result = \enrol_lticoursetemplate\helper::enrol_user($tool, $this->user2->id);

        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticoursetemplate\helper::ENROLMENT_MAX_ENROLLED, $result);
        $this->assertEquals(1, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_enrol_user_enrolment_not_started() {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticoursetemplate\helper::ENROLMENT_NOT_STARTED, $result);
    ...
    public function test_enrol_user_enrolment_finished() {
        ...
        // Check that this user was not enrolled and we are told why.
        $this->assertEquals(\enrol_lticoursetemplate\helper::ENROLMENT_FINISHED, $result);
        $this->assertEquals(0, $DB->count_records('user_enrolments', array('enrolid' => $tool->enrolid)));
    }
    ...
    public function test_count_lti_tools() {
        ...
        // Count all the tools.
        $count = \enrol_lticoursetemplate\helper::count_lti_tools();
        $this->assertEquals(4, $count);

        // Count all the tools in course 1.
        $count = \enrol_lticoursetemplate\helper::count_lti_tools(array('courseid' => $course1->id));
        ...
        $count = \enrol_lticoursetemplate\helper::count_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    public function test_get_lti_tools() {
        ...
        // Get all the tools.
        $tools = \enrol_lticoursetemplate\helper::get_lti_tools();

        // Check that we got all the tools.
        $this->assertEquals(4, count($tools));

        // Get all the tools in course 1.
        $tools = \enrol_lticoursetemplate\helper::get_lti_tools(array('courseid' => $course1->id));
        ...
        $tools = \enrol_lticoursetemplate\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED));
    ...
    protected function create_tool($data = array()) {
        ...
        $enrolplugin = enrol_get_plugin('lticoursetemplate');
        $instanceid = $enrolplugin->add_instance($course, (array) $data);

        // Get the tool associated with this instance.
        return $DB->get_record('enrol_lti_ct_tools', array('enrolid' => $instanceid));
    }
}
