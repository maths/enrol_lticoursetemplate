@enrol @enrol_lticoursetemplate
Feature: Check that settings are adhered to when creating an enrolment plugin
  In order to create an LTI Course Template enrolment instance
  As an admin
  ...
  Background:
    ...
    And I click on "Enable" "link" in the "Publish as LTI Course Template tool" "table_row"
    And I navigate to "Publish as LTI Course Template tool" node in "Site administration > Plugins > Enrolments"
  ...
  Scenario: As an admin set site-wide settings for the enrolment plugin and ensure they are used
    ...
    And I select "Publish as LTI Course Template tool" from the "Add method" singleselect
    ...
    And I click on "Edit" "link" in the "Publish as LTI Course Template tool" "table_row"
