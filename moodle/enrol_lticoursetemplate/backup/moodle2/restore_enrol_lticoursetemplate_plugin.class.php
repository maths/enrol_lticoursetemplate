...
class restore_enrol_lticoursetemplate_plugin extends restore_enrol_plugin {
    ...
    protected function define_enrol_plugin_structure() {
        ...
        $paths = array();
        $paths[] = new restore_path_element('enrol_lti_ct_tool', $this->connectionpoint->get_path() . '/cttool');
        $paths[] = new restore_path_element('enrol_lti_ct_users', $this->connectionpoint->get_path() . '/cttool/ctusers/user');
     ...
     */
    public function process_enrol_lti_ct_tool($data) {
        global $DB;
        ...
        // Set up the mapping.
        $this->set_mapping('enrol_lti_ct_tool', $oldid, $data->id);
    }
     ...
     */
    public function process_enrol_lti_ct_users($data) {
        global $DB;

        $data = (object) $data;

        $data->userid = $this->get_mappingid('user', $data->userid);
        $data->toolid = $this->get_mappingid('enrol_lti_ct_tool', $data->toolid);
        $data->timecreated = time();
