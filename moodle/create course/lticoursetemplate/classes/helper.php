...
class helper {
    ...
    public static function enrol_user($tool, $userid) {
        ...
        if (!$DB->record_exists('user_enrolments', array('enrolid' => $tool->enrolid, 'userid' => $userid))) {
            ...
            $ltienrol = enrol_get_plugin('lticoursetemplate');
    ...
    /**
     * Returns the LTI new tool and creates a new course.
     *
    ...
    public static function get_lti_new_tool($toolid, $consumerkey = NULL, $shortname = NULL, $fullname = NULL) {
        ...
        if ( !$course ) {
            ...
            // get the category id of the template course
        ...
        return self::get_lti_tool($tool->id); //return new tool
    }
