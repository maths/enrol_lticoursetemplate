...
class sync_members extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('tasksyncmembers', 'enrol_lticoursetemplate');
    }
    ...
    public function execute() {
        ...
        require_once($CFG->dirroot . '/enrol/lticoursetemplate/ims-blti/OAuthBody.php');
        ...
        if (!enrol_is_enabled('lticoursetemplate')) {
            mtrace('Skipping task - ' . get_string('enrolisdisabled', 'enrol_lticoursetemplate'));
            return true;
        }

        // Get all the enabled tools.
        if ($tools = \enrol_lticoursetemplate\helper::get_lti_tools(array('status' => ENROL_INSTANCE_ENABLED, 'membersync' => 1))) {
            ...
            foreach ($tools as $tool) {
                ...
                if ($ltiusers = $DB->get_records('enrol_lti_ct_users', array('toolid' => $tool->id), 'lastaccess DESC')) {
                    foreach ($ltiusers as $ltiuser) {
                        ...
                        foreach ($members as $member) {
                            ...
                            $user = \enrol_lticoursetemplate\helper::assign_user_tool_data($tool, $user);

                            if (!$dbuser = $DB->get_record('user', array('username' => $user->username, 'deleted' => 0))) {
                                if ($tool->membersyncmode == \enrol_lticoursetemplate\helper::MEMBER_SYNC_ENROL_AND_UNENROL ||
                                    $tool->membersyncmode == \enrol_lticoursetemplate\helper::MEMBER_SYNC_ENROL_NEW) {
                            ...
                            if ($tool->membersyncmode == \enrol_lticoursetemplate\helper::MEMBER_SYNC_ENROL_AND_UNENROL ||
                                ...
                                \enrol_lticoursetemplate\helper::enrol_user($tool, $user->id);
                            }
                        }
                    }
                    // Now we check if we have to unenrol users who were not listed.
                    if ($tool->membersyncmode == \enrol_lticoursetemplate\helper::MEMBER_SYNC_ENROL_AND_UNENROL ||
                        $tool->membersyncmode == \enrol_lticoursetemplate\helper::MEMBER_SYNC_UNENROL_MISSING) {
            ...
            if (!empty($userphotos)) {
                foreach ($userphotos as $userid => $url) {
                    if ($url) {
                        $result = \enrol_lticoursetemplate\helper::update_user_profile_image($userid, $url);
                        if ($result === \enrol_lticoursetemplate\helper::PROFILE_IMAGE_UPDATE_SUCCESSFUL) {
                            $counter++;
