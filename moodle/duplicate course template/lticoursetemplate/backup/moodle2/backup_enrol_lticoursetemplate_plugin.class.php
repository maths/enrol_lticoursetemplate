...
/**
 * Defines the backup_enrol_ltilticoursetemplate_plugin class.
 *
...
class backup_enrol_lticoursetemplate_plugin extends backup_enrol_plugin {
    ...
    */
    ...
    public function define_enrol_plugin_structure() {
        ...
        // Define our elements.
        $tool = new backup_nested_element('cttool', array('id'), array(
        ...
        $users = new backup_nested_element('ctusers');
        ...
        $tool->set_source_table('enrol_lti_ct_tools',
            array('enrolid' => backup::VAR_PARENTID));

        // Users are only added only if users included.
        if ($this->task->get_setting_value('users')) {
            $user->set_source_table('enrol_lti_ct_users', array('toolid' => backup::VAR_PARENTID));
        }
    }
}
