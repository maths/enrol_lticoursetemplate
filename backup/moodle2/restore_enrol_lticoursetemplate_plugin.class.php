...
class restore_enrol_lticoursetemplate_plugin extends restore_enrol_plugin {
    ...
    protected function define_enrol_plugin_structure() {
        ...
        $paths = array();
        $paths[] = new restore_path_element('enrol_ct_tool', $this->connectionpoint->get_path() . '/tool');
        $paths[] = new restore_path_element('enrol_ct_users', $this->connectionpoint->get_path() . '/tool/users/user');
     ...
     */
    public function process_enrol_ct_tool($data) {
        global $DB;
        ...
        $data->id = $DB->insert_record('enrol_ct_tools', $data);

        // Add the array of tools we need to process later.
        $this->tools[$data->id] = $data;

        // Set up the mapping.
        $this->set_mapping('enrol_ct_tool', $oldid, $data->id);
    }
     ...
     */
    public function process_enrol_ct_users($data) {
        global $DB;

        $data = (object) $data;

        $data->userid = $this->get_mappingid('user', $data->userid);
        $data->toolid = $this->get_mappingid('enrol_ct_tool', $data->toolid);
        $data->timecreated = time();

        $DB->insert_record('enrol_ct_users', $data);
    }
    ...
    public function after_restore_enrol() {
        ...
        foreach ($this->tools as $tool) {
            ...
            $DB->update_record('enrol_ct_tools', $updatetool);
        }
    }
}
