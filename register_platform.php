...
use core\output\notification;
use enrol_lticoursetemplate\local\ltiadvantage\form\create_registration_form;
use enrol_lticoursetemplate\local\ltiadvantage\form\platform_registration_form;
use enrol_lticoursetemplate\local\ltiadvantage\entity\application_registration;
...
use enrol_lticoursetemplate\local\ltiadvantage\service\application_registration_service;
...
$toolregistrationurl = new moodle_url($CFG->wwwroot . "/" . $CFG->admin . "/settings.php",
    ['section' => 'enrolsettingsltict_registrations']);
...
$pagesetup = function(string $pagetitle) {
    ...
    navigation_node::override_active_url(
        new moodle_url('/admin/settings.php', ['section' => 'enrolsettingsltict_registrations'])
    );
    admin_externalpage_setup('enrolsettingsltict_registrations_edit', '', null, '', ['pagelayout' => 'admin']);
...
if ($action == 'view') {
    ...
    global $PAGE;
    $pagesetup(get_string('registerplatformedit', 'enrol_lticoursetemplate'));
    $pageurl = new moodle_url('/enrol/lticoursetemplate/register_platform.php', ['action' => 'view', 'regid' => $regid]);

    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('registerplatformedit', 'enrol_lticoursetemplate'));
    ...
    $pagesetup(get_string('registerplatformadd', 'enrol_lticoursetemplate'));
    $pageurl = new moodle_url('/enrol/lticoursetemplate/register_platform.php', ['action' => 'add']);
    ...
    if ($data = $mform->get_data()) {
        ...
        redirect(new moodle_url('/enrol/lticoursetemplate/register_platform.php',
            ['action' => 'view', 'regid' => $draft->get_id(), 'tabselect' => 'tooldetails']));

    } else if (!$mform->is_cancelled()) {
        // Display the first step of registration creation.
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('registerplatformadd', 'enrol_lticoursetemplate'));
        $mform->display();
        echo $OUTPUT->footer();
        die();
    }
    ...
    $pageurl = new moodle_url('/enrol/lticoursetemplate/register_platform.php', ['action' => 'edit', 'regid' => $regid]);
    $viewurl = new moodle_url('/enrol/lticoursetemplate/register_platform.php', ['action' => 'view', 'regid' => $regid]);
    ...
    if (($data = $mform->get_data()) && confirm_sesskey()) {
        ...
        redirect($viewurl, get_string('registerplatformeditnotice', 'enrol_lticoursetemplate'), null,
    ...
    $regid = required_param('regid', PARAM_INT);
    $pagesetup(get_string('registerplatformdelete', 'enrol_lticoursetemplate'));

    if (!optional_param('confirm', false, PARAM_BOOL)) {
        $continueparams = ['action' => 'delete', 'regid' => $regid, 'sesskey' => sesskey(), 'confirm' => true];
        $continueurl = new moodle_url('/enrol/lticoursetemplate/register_platform.php', $continueparams);
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->confirm(
            get_string('registerplatformdeleteconfirm', 'enrol_lticoursetemplate', format_string($appreg->get_name())),
            $continueurl,
            $toolregistrationurl
        );
        echo $OUTPUT->footer();
        ...
        redirect($toolregistrationurl,
            get_string('registerplatformdeletenotice', 'enrol_lticoursetemplate'), null,  notification::NOTIFY_SUCCESS);
    }
}
