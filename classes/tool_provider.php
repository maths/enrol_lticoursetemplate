...
use core\notification;
use core_user;
use enrol_lticoursetemplate\output\registration;
use html_writer;
...
class tool_provider extends ToolProvider {
    ...
    protected function onError() {
        ...
        if (empty($this->returnUrl)) {
            $this->errorOutput = $OUTPUT->notification(get_string('failedrequest', 'enrol_lticoursetemplate', ['reason' => $message]), 'error');
        }
    }
    ...
    protected function onLaunch() {
        ...
        if (empty($this->consumer) || $this->dataConnector->loadToolConsumer($this->consumer) === false) {
            $this->ok = false;
            $this->message = get_string('invalidtoolconsumer', 'enrol_lticoursetemplate');
            return;
        }
        ...
        if (!$correctlaunchurl) {
            $this->ok = false;
            $this->message = get_string('invalidrequest', 'enrol_lticoursetemplate');
            return;
        }
        ...
        if ($userlog = $DB->get_record('enrol_ct_users', ['toolid' => $tool->id, 'userid' => $user->id])) {
    ...
    protected function onRegister() {
        ...
        if (empty($this->returnUrl)) {
            $this->ok = false;
            $this->message = get_string('returnurlnotset', 'enrol_lticoursetemplate');
            return;
        }
        ...
        if ($this->doToolProxyService()) {
            ...
            $this->message = get_string('successfulregistration', 'enrol_lticoursetemplate');

            // Prepare response.
            $returnurl = new moodle_url($this->returnUrl);
            $returnurl->param('lti_msg', get_string("successfulregistration", "enrol_lticoursetemplate"));
            ...
            $output = $PAGE->get_renderer('enrol_lticoursetemplate');
            echo $output->render($registration);

        } else {
            // Tell the consumer that the registration failed.
            $this->ok = false;
            $this->message = get_string('couldnotestablishproxy', 'enrol_lticoursetemplate');
        }
    }
    ...
    public function map_tool_to_consumer() {
        ...
        if (empty($this->consumer)) {
            throw new moodle_exception('invalidtoolconsumer', 'enrol_lticoursetemplate');
        }
        ...
        $mappingexists = $DB->record_exists('enrol_ct_tool_cons_map', $mappingparams);
        if (!$mappingexists) {
            $DB->insert_record('enrol_ct_tool_cons_map', (object) $mappingparams);
        }
    }
}
