...
/**
 * Renderer class for LTIcoursetemplate enrolment
 *
...
namespace enrol_lticoursetemplate\output;
...
class renderer extends plugin_renderer_base {
    /**
     * Render the enrol_lticoursetemplate/proxy_registration template
     *
    ...
    public function render_registration(registration $registration) {
        ...
        return parent::render_from_template("enrol_lticoursetemplate/proxy_registration", $data);
    }
     ...
     * a list of LTIcoursetemplate Resource Link Content Items being sent back to the platform, allowing resource link creation to
     * take place.
     *
    ...
    public function render_published_resource_selection_view(LtiMessageLaunch $launch, array $resources): string {
        global $CFG;
        $context = [
            'action' => $CFG->wwwroot . '/enrol/lticoursetemplate/configure.php',
        ...
        return parent::render_from_template('enrol_lticoursetemplate/local/ltiadvantage/content_select', $context);
    }

    /**
     * Render the table applications which have been registered as LTIcoursetemplate Advantage platforms.
     *
    ...
    public function render_admin_setting_registered_platforms(array $registrations): string {
        $registrationscontext = [
            'registrations' => [],
            'addurl' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php', ['action' => 'add']))->out(false),
        ];
        ...
        foreach ($registrations as $reg) {
            ...
            $status = get_string('registrationstatuspending', 'enrol_lticoursetemplate');
            if ($reg->is_complete()) {
                $status = get_string('registrationstatusactive', 'enrol_lticoursetemplate');
            }
            $registrationscontext['registrations'][] = [
                ...
                'statusstring' => $status,
                'tooldetailsurl' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'tooldetails']))->out(false),
                'platformdetailsurl' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'platformdetails']))->out(false),
                'deploymentsurl' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php',
                    ['action' => 'view', 'regid' => $reg->get_id(), 'tabselect' => 'tooldeployments']))->out(false),
                'deleteurl' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php',
        ...
        $versionnotice = new notification(
            get_string('registeredplatformsltiversionnotice', 'enrol_lticoursetemplate'),
            notification::NOTIFY_INFO
        );
        $versionnotice->set_show_closebutton(false);

        $return = parent::render($versionnotice);
        $return .= parent::render_from_template('enrol_lticoursetemplate/local/ltiadvantage/registered_platforms',
            $registrationscontext);
        return $return;
    }
     ...
     * Valid values correspond to the tab names in the enrol_lticoursetemplate/local/ltiadvantage/registration_view template, currently:
    ...
    public function render_registration_view(int $registrationid, string $activetab = '') {
        ...
        foreach ($deployments as $deployment) {
            $deploymentscontext[] = [
                ...
                'deleteurl' => (new \moodle_url(
                    '/enrol/lticoursetemplate/manage_deployment.php',
                    ['action' => 'delete', 'id' => $deployment->get_id(), 'registrationid' => $registration->get_id()]
                ))->out(false)
            ];
        }

        $regurl = new \moodle_url('/enrol/lticoursetemplate/register.php', ['token' => $registration->get_uniqueid()]);
        ...
        $tcontext = [
            ...
            'back_url' => (new \moodle_url('/admin/settings.php', ['section' => 'enrolsettingsltict_registrations']))->out(false),
            ...
            'dynamic_registration_url' => [
                'name' => get_string('registrationurl', 'enrol_lticoursetemplate'),
                'url' => $regurl,
                'id' => uniqid()
            ],
            'manual_registration_info' => get_string('endpointltiversionnotice', 'enrol_lticoursetemplate'),
            'manual_registration_urls' => [
                [
                    'name' => get_string('toolurl', 'enrol_lticoursetemplate'),
                    'url' => $CFG->wwwroot . '/enrol/lticoursetemplate/launch.php',
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('loginurl', 'enrol_lticoursetemplate'),
                    'url' => $CFG->wwwroot . '/enrol/lticoursetemplate/login.php?id=' . $registration->get_uniqueid(),
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('jwksurl', 'enrol_lticoursetemplate'),
                    'url' => $CFG->wwwroot . '/enrol/lticoursetemplate/jwks.php',
                    'id' => uniqid()
                ],
                [
                    'name' => get_string('deeplinkingurl', 'enrol_lticoursetemplate'),
                    'url' => $CFG->wwwroot . '/enrol/lticoursetemplate/launch_deeplink.php',
                    'id' => uniqid()
                ],
            ],
            'platform_details_info' => get_string('platformdetailsinfo', 'enrol_lticoursetemplate'),
            'platform_details' => [
                [
                    'name' => get_string('registerplatform:name', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_name()
                ],
                [
                    'name' => get_string('registerplatform:platformid', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_platformid() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:clientid', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_clientid() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:authrequesturl', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_authenticationrequesturl() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:jwksurl', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_jwksurl() ?? '',
                ],
                [
                    'name' => get_string('registerplatform:accesstokenurl', 'enrol_lticoursetemplate'),
                    'value' => $registration->get_accesstokenurl() ?? '',
                ]
            ],
            'edit_platform_details_url' => (new \moodle_url('/enrol/lticoursetemplate/register_platform.php',
                ['action' => 'edit', 'regid' => $registration->get_id()]))->out(false),
            'deployments_info' => get_string('deploymentsinfo', 'enrol_lticoursetemplate'),
            'has_deployments' => !empty($deploymentscontext),
            'tool_deployments' => $deploymentscontext,
            'add_deployment_url' => (new \moodle_url('/enrol/lticoursetemplate/manage_deployment.php',
                ['action' => 'add', 'registrationid' => $registrationid]))->out(false)
        ];

        return parent::render_from_template('enrol_lticoursetemplate/local/ltiadvantage/registration_view',
            $tcontext);
    }
}
