...
namespace enrol_lticoursetemplate\local\ltiadvantage\service;

use enrol_lticoursetemplate\helper;
use enrol_lticoursetemplate\local\ltiadvantage\entity\context;
use enrol_lticoursetemplate\local\ltiadvantage\entity\deployment;
use enrol_lticoursetemplate\local\ltiadvantage\entity\migration_claim;
...
class tool_launch_service {
    ...
    public function user_launches_tool(\stdClass $user, LtiMessageLaunch $launch): array {
        ...
        if (!$registration = $this->registrationrepo->find_by_platform($launchdata->platform, $launchdata->clientid)) {
            throw new \moodle_exception('ltiadvlauncherror:invalidregistration', 'enrol_lticoursetemplate', '',
        ...
        if (!$deployment = $this->deploymentrepo->find_by_registration($registration->get_id(),
            ...
            throw new \moodle_exception('ltiadvlauncherror:invaliddeployment', 'enrol_lticoursetemplate', '',
                [$launchdata->deploymentid]);
        }
        ...
        if ($result !== helper::ENROLMENT_SUCCESSFUL) {
            throw new \moodle_exception($result, 'enrol_lticoursetemplate');
        }
