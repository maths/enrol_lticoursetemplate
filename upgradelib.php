 ...
 */
function enrol_lticoursetemplate_verify_private_key() {

    $name = 'lti_13_kid';
    $key = get_config('enrol_lticoursetemplate', $name);

    // If we already generated a valid key, no need to check.
    if (empty($key)) {
        // Create the private key.
        $kid = bin2hex(openssl_random_pseudo_bytes(10));
        set_config($name, $kid, 'enrol_lticoursetemplate');
        $config = array(
            "digest_alg" => "sha256",
        ...
        if (!empty($privatekey)) {
            set_config('lti_13_privatekey', $privatekey, 'enrol_lticoursetemplate');
        } else {
            return get_string('opensslconfiginvalid', 'enrol_lticoursetemplate');
        }
    }
